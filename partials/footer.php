        ...
        <div class="row">
            ...
            <div class="col-xl-6 col-lg-6 col-md-6">
                <div class="it-newsletter-right p-relative text-end">
                    <input type="email" placeholder="Enter your email:" id="nl-input">
...
<script>
    $(document).ready(function() {
        ...
        $("#nl-button").on('click', function() {
            var val = $('#nl-input').val()
            if (val) {
                console.log(val)
                ...
                $(".nl-alert").addClass('d-none');
                fetch('/send-newsletter.php', requestOptions)
                    .then(response => {
                        if (response.ok) {
                            // response.text().then(data => console.log(data))
                            $(".nl-alert.alert-success").removeClass('d-none');
                        } else {
                            // response.text().then(data => console.log(data))
                            $(".nl-alert.alert-danger").removeClass('d-none');
                        }
                    }).catch(error => {
                        // console.log(error)
                        $(".nl-alert.alert-danger").removeClass('d-none');
